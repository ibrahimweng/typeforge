...
export function catalogue(shell: Shell): Item[] {
  /*
   * The verbs, the screens and the open fonts come from `actions.ts`, which the
   * menu bar reads as well. See the note at the top of it: two things show this
   * list now, and a second copy of it would go wrong quietly.
   */
  const items: Item[] = appActions(shell);
