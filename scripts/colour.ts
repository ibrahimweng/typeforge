...
for (const weight of weights) {
  for (const reach of reaches.length ? reaches : [base.parts.script.reach]) {
    ...
    for (const ch of TEXT) {
      ...
      if (!d) continue;
      parts.push(
        `<g transform="translate(${x} 0)"><path d="${contoursToSvgPath(d.contours)}" fill="#111"/></g>`,
      );
      x += d.advanceWidth;
    }
...
writeFileSync(
  process.env.OUT ?? "/tmp/wt.html",
  `<body style="margin:18px;background:#fff">${blocks.join("")}</body>`,
);
