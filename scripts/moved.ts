...
if (process.env.COMPARE) {
  ...
  for (const name of Object.keys(a)) {
    ...
    if (p.length !== q.length) {
      rows.push([`${name} (NODE COUNT ${p.length / 2} -> ${q.length / 2})`, Infinity]);
      continue;
    }
    let worst = 0;
    for (let i = 0; i < p.length; i += 2)
      worst = Math.max(worst, Math.hypot(p[i] - q[i], p[i + 1] - q[i + 1]));
  ...
  for (const [name, worst] of rows.slice(0, 20))
    console.log(
      `  ${name.padEnd(18)} ${worst === Infinity ? "structure" : `${worst.toFixed(1)} units`}`,
    );
} else {
  ...
  for (const glyph of typeface.glyphs) {
    ...
    for (const contour of glyph.contours)
      for (const node of contour.nodes) flat.push(node.point.x, node.point.y);
    out[glyph.name] = flat;
  }
